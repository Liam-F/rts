...
#print(index)
columns = ['Minus32','Minus17','Ref','Delta32to17','DrawDownMax','DrawUpMax']
df_ = pd.DataFrame(index=index, columns=columns)
df_ = df_.fillna(0) # with 0s rather than NaNs
data = np.array([np.arange(len(index))]*6).T
...
for idx, row in df.iterrows():
    ...
    df.loc[dRef] = pd.Series({'Minus32':p32, 'Minus17':p17, 'Ref':pRef, 'Delta32to17':pdelta32to17,'DrawDownMax':pDraw_downmax,'DrawUpMax':pDraw_upmax})
