...
#        df_00.set_value(np.where(df_00['openlongoptsym']==row['openlongoptsym'])[0][0], 'statcloselongbid', df_optionpricescurrent[(df_optionpricescurrent.optionsymbol == row['openlongoptsym'])].bid.iloc[0])
    ...
    df_ana = df_01.rename(columns={'openshortopenlongoptsym': 'openshortopenlongoptsym', ('stkp', 'amin'): 'openstkp',('stattime', 'amin') : 'stattime',('captureopen', 'sum'): 'captureopen',('captureclose', 'sum'): 'captureclose',('statsp', 'amin') :'statstkp'})        
    print(df_ana)
    #print 
    import mytools
    ...
    if os.path.isfile(filepath_to_statusofcandidates) != True:
        df_ana.to_csv(filepath_to_statusofcandidates,columns=('openshortopenlongoptsym', 'openstkp','stattime','captureopen','captureclose','statstkp'))
        ...
        df_ana.to_csv(existingcsvfile, header = False)
