...
cachefolder = config.cachefolder #'C:\\Batches\\rts\\output\\cache'
...
#        print('++++++++++++++++++++++++more than 10',row)
    ...
    if optiontype == 'C':
        if pdeltapct_to_sell_price > 0:
            ...
            if str(pdeltapct_atthresholdf_calloption) == 'nan':
...
#        sum_of_iter_capt_at_cumprob_cross = sum_of_iter_capt_at_cumprob_cross - float(prevcandidaterow[a_indices[0]])
        ...
        #candidaterows.append(candidateheader)
        candidaterows.append([myoptiontype,sellstockprice,pdeltapct_to_sell_price,sellstrike,buystrike,sellcumprob,buycumprob,sellbidprice,buyaskprice,round(float(sellbidprice)-float(buyaskprice),3),myexdate.strftime('%Y-%m-%d'),datestringforcsv,daysbackmid,compare_stock_price,number_of_observations,sel])
...
'''@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                                Candidate Value CSV
   @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'''
df_candidates.to_csv(mysymboloutputfolder + "\\ironcondor candidates (" + expirationdate_string + ') ' + symbol +  " " + datestringforfilename + ".csv",columns=('ty','stkp','spdel','sstrk','bstrk','scump','bcump','sbid','bask','capt','myexdate','currdate','daysbackmid','pricecompare','obsv','sel'))
...
filepath_to_selectedcandidates = os.path.join(myselectedcandidatesfolder,'selectedcandidates ' + expirationdate_string + ' ' + symbol + '.csv')
#import os
...
if os.path.isfile(filepath_to_selectedcandidates) != True:
    df_selectedcandidates.to_csv(filepath_to_selectedcandidates,columns=('ty','stkp','spdel','sstrk','bstrk','scump','bcump','sbid','bask','capt','myexdate','currdate','daysbackmid','pricecompare','obsv','sel'))
