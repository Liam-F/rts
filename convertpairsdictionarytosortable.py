...
#    SortbyMeasure = property(get_SortbyMeasure, set_SortbyMeasure)    
    ...
    def execute_convert(self,dictionaryofpairs,sortbymeasure,showresults):
        ...
        for k1,v1 in dictionaryofpairs.items():
            ...
            earlier = v1[0]
            later = v1[1]
            spreadpercentageopen = float(earlier.bid)/float(later.ask)
            valueatriskopen = -float(earlier.bid) + float(later.ask)
            
            if sortbymeasure == 'valueatriskopen':
                dSortableDictionary[len(dSortableDictionary)] = {'sortbymeasurevalue' : valueatriskopen, 'earlier' : earlier, 'later' : later, 'origkey' : k1, 'sortbymeasurename' : 'valueatriskopen'}
            else:
                dSortableDictionary[len(dSortableDictionary)] = {'sortbymeasurevalue' : spreadpercentageopen, 'earlier' : earlier, 'later' : later, 'origkey' : k1, 'sortbymeasurename' : 'spreadpercentageopen'}
        #######################################################################
