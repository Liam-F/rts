...
symbol = 'SPY'
myspreadindollars = 1
mycumprob_lowrange = 0
mycumprob_highrange = 95
expirationdate_string = '2015-07-02'
...
rows.append(['exdate','symbol','ty','st','strike_at_sell_price','pdeltapct','cumprob','bid','ask','iv','iscandidate'])
d_candidates = {}
for index, row in df.iterrows():
    ...
    pdeltapct = (float(strike_at_sell_price) - float(stockprice)) / float(stockprice)
    cumprob = float('NaN')
    iscandidate = 0
    # ========================
...
#        print('++++++++++++++++++++++++more than 10',row)
    ...
    if cumprob != float('NaN'):
        if cumprob > mycumprob_lowrange and cumprob < mycumprob_highrange:
            iscandidate = 1
            # ##########
        ...
        rows.append([exdate,symbol,optiontype,stockprice,strike_at_sell_price,'{percent:.2%}'.format(percent=pdeltapct),'{percent:.2%}'.format(percent=cumprob/100),row['bid'],row['ask'],row['impliedvolatility'],iscandidate])
        #d_candidates[strike_at_sell_price,optiontype] = [symbol,optiontype,stockprice,strike_at_sell_price,'{percent:.2%}'.format(percent=pdeltapct),round(cumprob,1),row['bid'],row['ask'],row['impliedvolatility'],iscandidate]
        
    #print(symbol,'price change from ',stockprice,'to strike_at_sell_price',strike_at_sell_price,'(','{percent:.2%}'.format(percent=pdeltapct),') exp',exdate,cumprob)
headers = rows.pop(0)
...
candidaterows = []        
candidaterows.append(['ty','stkp','spdel','sstrk','bstrk','scump','sbid','bask','capt','myexdate'])
            ...
            for index_inner,row_inner in df_01.iterrows():
                if row_inner['ty'] == row['ty']:
                    if float(row_inner['strike_at_sell_price']) == float(row['strike_at_sell_price'])-myspreadindollars:
                        ...
                        buystrike = row_inner['strike_at_sell_price']
                        #print('PUT >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
        candidaterows.append([myoptiontype,sellstockprice,pdeltapct,sellstrike,buystrike,sellcumprob,sellbidprice,buyaskprice,round(float(sellbidprice)-float(buyaskprice),3),myexdate.strftime('%Y-%m-%d')])
        #print(myexdate.strftime('%Y-%m-%d'),myoptiontype,sellstockprice,pdeltapct,sellstrike,buystrike,sellcumprob,sellbidprice,buyaskprice,round(float(sellbidprice)-float(buyaskprice),3))
        #print(row)
